	...
	<div id="wrapper">
		...
		<div class="navbar-header">
			<a class="navbar-brand waves-effect waves-dark"
				href="BgDonationServlet"> <strong>后台管理系统</strong>
			</a>
									...
									<table class="table table-striped table-bordered table-hover"
										...
										<tbody>
											<c:forEach items="${users}" var="user">
												<tr>
													<td>${user.cell_phone_id }</td>
													<td><a
														href="showVolunteerDetail.jsp?user_id=${user.user_id }">点击查看</a></td>
													<td>
														<form class="left" method="post"
															action="VirifyVolunteerServlet">
															<input type="hidden" value=${user.user_id }
																name="volunteer" /><input type="hidden" value="1"
														...
														<form class="left" method="post"
															action="VirifyVolunteerServlet">
															<input type="hidden" value=${user.user_id }
																name="volunteer" /><input type="hidden" value="2"
																name="virify" /><input type="submit"
